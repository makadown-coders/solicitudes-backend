...
class KitsClavesController {
    ...
    async listByKit(req: Request, res: Response): Promise<void> {
        try {
    ...
    async addClave(req: Request, res: Response): Promise<void> {
        try {
            const kitId = parseInt(req.params.kitId, 10);
            const { clave, aplica } = req.body;
            if (!kitId || !clave) {
                res.status(400).json({ ok: false, msg: 'kitId/clave requeridos' });
                return;
            }
    ...
    async deleteClave(req: Request, res: Response): Promise<void> {
        try {
    ...
    listByCodigo = async (req: Request, res: Response): Promise<void> => {
        try {
            const codigo = req.params.codigo;
            if (!codigo) {
                res.status(400).json({ ok: false, msg: 'código requerido' });
                return;
            }
