// src/controllers/historiales.controller.ts
...
class HistorialesController {
    ...
    // NUEVO: recibe la encuesta del frontend y la manda al Flow de encuesta
  async postEncuestaPiloto(req: Request, res: Response) {
    try {
      ...
      for (const k of required) {
        if ((b as any)[k] === undefined || (b as any)[k] === null) {
          return res.status(400).json({ error: `Falta campo requerido: ${k}` });
        }
      }
      if (Number.isNaN(Date.parse(String(b.timestamp)))) {
        return res.status(400).json({ error: 'timestamp inválido (ISO requerido)' });
      }
      const f = Number(b.facilidad_1_5);
      const c = Number(b.csat_1_5);
      if (!Number.isInteger(f) || f < 1 || f > 5) {
        return res.status(400).json({ error: 'facilidad_1_5 debe ser entero 1..5' });
      }
      if (!Number.isInteger(c) || c < 1 || c > 5) {
        return res.status(400).json({ error: 'csat_1_5 debe ser entero 1..5' });
      }
      if (typeof b.termino_sin_trabas !== 'boolean') {
        return res.status(400).json({ error: 'termino_sin_trabas debe ser boolean' });
      }
      if (b.comentario && String(b.comentario).length > 500) {
        return res.status(400).json({ error: 'comentario excede 500 caracteres' });
      }
      ...
      console.error(err);
      return res.status(500).json({ error: 'Error al enviar encuesta a Power Automate' });
    }
  }
}
