...
interface CitaRow {
  ...
  16?: string | Date | null;   // fechaLimiteEntrega
  ...
  try {
    ...
    if (count === 0) {
      ...
      for (const fila of rows) {
        ...
        const carga = fila[26];
        // columnas 27 y 28 no se usan en el excel
        const fechaCita = fila[29];
        ...
        await client.query(
          ...
          [ejercicio,
            ...
            fechaLimiteEntrega instanceof Date ? fechaLimiteEntrega : (fechaLimiteEntrega as string | null),
            piezasRecibidas !== null && piezasRecibidas !== undefined ? Number(piezasRecibidas) : null,
            fechaRecepcionAlmacen instanceof Date ? fechaRecepcionAlmacen : (fechaRecepcionAlmacen as string | null),
            numeroRemision,
            lote,
            caducidad instanceof Date ? caducidad : (caducidad as string | null),
            estatus,
            folioAbasto,
            almacenHospital,
            evidencia,
            carga,
            fechaCita instanceof Date ? fechaCita : (fechaCita as string | null),
            observacion]
        );
      }
    ...
    console.error('❌ Error al ejecutar el seed de citas:', err.message);
  } finally {
    if (client) {
      client.release();
    }
